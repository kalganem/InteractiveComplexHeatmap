...
$(function() {
	...
	if('@{output_ui_float}' == 'TRUE') {
		...
		$('#@{heatmap_id}_output_wrapper').css({'display': 'block', 'position':'absolute','right':'auto','left':'auto'});
	    if('@{action}' == "hover") {
		...
		if('@{action}' == "click") {
	    	$('#@{heatmap_id}_heatmap').click(function(e) {
		    	$('#@{heatmap_id}_output_wrapper').css({'display':'block','position':'absolute', 'left':e.pageX+5, 'right':'auto', 'top':e.pageY+5});
